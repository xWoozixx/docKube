# Commands to run only on the Control-Plane
...
# Récupérer le paquet calico.yml
...
sed -i '0,/# - name: CALICO_IPV4POOL_CIDR/s/# - name: CALICO_IPV4POOL_CIDR/- name: CALICO_IPV4POOL_CIDR/' calico.yaml
sed -i '/- name: CALICO_IPV4POOL_CIDR/{n;s/#   value: "192.168.0.0\/16"/  value: "10.254.0.0\/16"/}' calico.yaml

#You can now just use kubeadm init to bootstrap the cluster
#add the --kubernetes-version 1.27.x parameter if you want to choose a specific version.
...
#crée le fichier de conf d'ingress
...
#Déploie ingress et crée un second pod pour avoir une redondance
